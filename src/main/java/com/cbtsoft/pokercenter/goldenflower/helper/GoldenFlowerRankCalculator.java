...
public class GoldenFlowerRankCalculator implements PokerRankCalculator {
    ...
    @Override
    public int rank(List<Card> originCards) {

        if (originCards.size() != 3) {
            throw new IllegalArgumentException("扎金花是三张牌的游戏!");
        }

        List<Card> cards = new ArrayList<>();
        for (Card card : originCards) {
